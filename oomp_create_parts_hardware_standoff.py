...
def load_parts(**kwargs):
    ...
    if True:
        extras_sizes = {}
        extras_sizes["m3"] = [6,9,10,12,15,20,25,30]
        # bolt
        for size in extras_sizes:
            ...
            part_details["color"] = ["nylon_white","black","brass"]
            lengths = extras_sizes[size]
            part_details["description_main"] = []
            for length in lengths:
                part_details["description_main"].append(f"{length}_mm_length")
            part_details["description_extra"] = ["nut_and_nut", "screw_and_nut","screw_and_screw"]
